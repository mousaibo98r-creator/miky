...
with col_profile:
    ...
    if event and event.selection.rows:
        ...
        st.markdown(
            f"<div style='font-size:1.5rem;font-weight:bold;color:#00d2ff;border-bottom:2px solid #00d2ff;padding-bottom:10px'>{record['buyer_name']}</div>",
        ...
        st.markdown(
            f"**\U0001f4e7 Email:** {', '.join(emails) if isinstance(emails, list) else str(emails)}"
        )

        phones = record.get("phone", [])
        st.markdown(
            f"**\U0001f4de Phone:** {', '.join(phones) if isinstance(phones, list) else str(phones)}"
        )

        websites = record.get("website", [])
        st.markdown(
            f"**\U0001f310 Website:** {', '.join(websites) if isinstance(websites, list) else str(websites)}"
        )
        ...
        with b1:
            st.link_button(
                "\U0001f310 Google Search",
                f"https://www.google.com/search?q={record['buyer_name']}",
            )
        with b2:
