...
if df is not None:
    ...
    df_b = pd.DataFrame({
        'Country': countries,
        'Value': [random.randint(100, 1000) for _ in range(len(countries))]
    })

with col1:
    st.markdown(f'<div style="background:#262730;padding:20px;border-radius:10px;border:1px solid #41424b;text-align:center"><div style="font-size:1rem;color:#fafafa">Total Volume (USD)</div><div style="font-size:2rem;font-weight:bold;color:#00d2ff">{total_rev}</div></div>', unsafe_allow_html=True)
with col2:
    st.markdown(f'<div style="background:#262730;padding:20px;border-radius:10px;border:1px solid #41424b;text-align:center"><div style="font-size:1rem;color:#fafafa">Active Buyers</div><div style="font-size:2rem;font-weight:bold;color:#00d2ff">{active_buyers}</div></div>', unsafe_allow_html=True)
with col3:
    st.markdown(f'<div style="background:#262730;padding:20px;border-radius:10px;border:1px solid #41424b;text-align:center"><div style="font-size:1rem;color:#fafafa">YoY Growth</div><div style="font-size:2rem;font-weight:bold;color:#00d2ff">{growth}</div></div>', unsafe_allow_html=True)
